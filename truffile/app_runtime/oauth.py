...
from .errors import AppAuthError
...
class OAuth:
    ...
    def _load_serialized_from_app_var(self) -> str | None:
        ...
        try:
            ...
            # app vars are the durable token store; falling back to the
            # container-local token file means we may be using stale creds
            LOGGER.warning(
                "failed to load %s from app vars (falling back to token file; APP_SESSION_TOKEN may be stale): %s",
                self.APP_VAR_KEY,
                exc,
            )
            return None
    ...
    def _save_serialized_to_app_var(self, serialized: str) -> None:
        ...
        try:
            ...
            # a refreshed token that only lands in the container file is lost
            # when the container is torn down (rotating refresh tokens break)
            LOGGER.warning("failed to save %s to app vars (token persists only in this container): %s", self.APP_VAR_KEY, exc)
