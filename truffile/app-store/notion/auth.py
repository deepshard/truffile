...
from config import (
    NOTION_AUTH_TOKEN_ENDPOINT,
    NOTION_OAUTH_APP_VAR_KEY,
    NOTION_OAUTH_RESOURCE,
    resolve_token_file,
)
...
class NotionAuth(RemoteMcpOAuth):
    def __init__(self, token_file: Path | None = None, read_only: bool = False) -> None:
        ...
        env_access_token = str(os.getenv("NOTION_ACCESS_TOKEN", "") or "").strip()
        super().__init__(
            resolved,
            app_var_key=NOTION_OAUTH_APP_VAR_KEY,
            env_access_token=env_access_token,
            default_token_endpoint=NOTION_AUTH_TOKEN_ENDPOINT,
            default_resource=NOTION_OAUTH_RESOURCE,
            read_only=read_only,
        )
